...
ws.on('message', (message: string) => {
    ...
    if (text.startsWith('>> Client')) {
        const systemMatch = text.match(/>> Client (\d+)/);
        const systemNumber = systemMatch ? parseInt(systemMatch[1]) : 0;
        const systemColor = getColorFromNumber(systemNumber);

        console.log(systemColor(text));
    } else {
